...
def test_clean_po_swaps_bad_dates():
    po = pd.DataFrame({
        "po_id": ["PO-1"], "sku": ["P101"], "supplier_id": ["S1"],
...
def test_validation_flags_unknown_sku_and_negatives():
    ...
    inv = pd.DataFrame({"date": sales["date"], "sku": "P101",
                        "on_hand": [50] * 10, "on_order": [0] * 10})
    inv.loc[2, "on_hand"] = -3
    products = pd.DataFrame({"sku": ["P101"], "name": ["Bolt"], "category": ["F"],
                             "unit_cost": [4.0], "pack_size": [10]})
    suppliers = pd.DataFrame({"supplier_id": ["S1"], "name": ["A"], "country": ["X"],
                              "target_otd": [0.9]})
    terms = pd.DataFrame({"sku": ["P101"], "supplier_id": ["S1"], "unit_price": [4.5],
                          "quoted_lead_days": [5], "lead_time_std": [1.0],
                          "is_primary": [True]})
    pos = pd.DataFrame({"po_id": ["PO-1"], "sku": ["P101"], "supplier_id": ["S1"],
                        ...
                        "quantity": [50.0], "unit_price": [4.5]})

    from supplychainxai.data.ingest import RawData
    rep = validate_all(RawData(products=products, suppliers=suppliers,
                               supply_terms=terms, sales=sales, inventory=inv,
                               purchase_orders=pos))
...
def test_features_lags_and_no_leakage():
    ...
    inv = pd.DataFrame({"date": sales["date"], "sku": "P101",
                        "on_hand": [100 - i for i in range(10)], "on_order": [0] * 10})
...
def test_model_frame_drops_nan_head():
    ...
    sales = pd.DataFrame({
        ...
        "sku": "P101", "units_sold": range(10, 10 + n),
        "unit_price": [5.0] * n, "promo_flag": [0] * n,
    })
    inv = pd.DataFrame({"date": sales["date"], "sku": "P101",
                        "on_hand": [10] * n, "on_order": [0] * n})
    feats = build_features(sales, inv)
