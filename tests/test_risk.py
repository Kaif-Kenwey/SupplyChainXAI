...
from supplychainxai.risk.engine import (
    demand_spikes,
    effective_lead,
...
def _sales_frame(values, end="2025-12-31"):
    ...
    return pd.DataFrame(
        {"date": dates, "sku": "P101", "units_sold": values, "unit_price": 5.0, "promo_flag": 0}
    )
...
def test_lead_time_intelligence_detects_drift():
    ...
    leads = [12] * 20 + [18] * 10
    po["delivered_date"] = [o + pd.Timedelta(days=lead) for o, lead in zip(po["order_date"], leads)]
...
def test_supplier_reliability_small_sample_guard():
    ...
    suppliers = pd.DataFrame(
        {"supplier_id": ["S2"], "name": ["X"], "country": ["Y"], "target_otd": [0.9]}
    )
