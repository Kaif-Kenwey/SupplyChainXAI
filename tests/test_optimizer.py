    ...
    products = pd.DataFrame(
        {
            ...
            "category": ["F", "S"],
            "unit_cost": [4.0, 8.0],
            "base_demand": [100.0, 50.0],
            "trend_per_year": [0.0, 0.0],
            "pack_size": [10, 5],
        }
    )
    suppliers = pd.DataFrame(
        {
            ...
            "name": ["Apex", "CheapCo", "Relia"],
            "country": ["IN", "CN", "DE"],
            "target_otd": [0.95, 0.70, 0.98],
        }
    )
    terms = pd.DataFrame(
        {
            ...
            "quoted_lead_days": [7, 10, 14],
            "lead_time_std": [1.0, 3.0, 2.0],
    ...
    pos = pd.DataFrame(
        {
            ...
            "supplier_id": ["S1"],
            "order_date": [dates[0]],
            "expected_date": [dates[0] + pd.Timedelta(days=7)],
...
def test_choose_supplier_penalises_unreliable_cheap_option():
    ...
    sup_stats = pd.DataFrame(
        [
            {"supplier_id": "S1", "on_time_rate": 0.95, "avg_lead": 7.0},
...
def test_simulator_demand_increase_raises_procurement():
    ...
    assert (
        up["summary"]["avg_stockout_probability"]
        >= base["summary"]["avg_stockout_probability"] - 0.02
    )
...
def test_simulator_lead_time_slip_increases_stockout_probability():
    ...
    assert (
        slip["summary"]["avg_stockout_probability"]
        >= base["summary"]["avg_stockout_probability"] - 0.02
    )
