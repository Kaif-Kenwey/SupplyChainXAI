...
def explain_forecast(
    ...
    local_drivers: dict | None = None,
) -> dict:
    """Forecast explanation: WHY did the model forecast this demand?

    When `shap_importance` is supplied (global mean |SHAP| from the tree
    explainer) it takes precedence over permutation importance for the
    feature shares; `local_drivers` (per-day SHAP contributions for the
    horizon's first input row) is passed through verbatim. Both are
    optional — the explanation works identically without them.

    This surface is deliberately SEPARATE from the BUY decomposition in
    `explain_recommendation`: SHAP explains the model's forecast, the
    additive decomposition explains the procurement recommendation.
    """
    ...
    top_features = []
    method = None
    # SHAP global attributions (tree models) take precedence when available
    if shap_importance is not None and len(shap_importance):
        imp = (
            shap_importance[shap_importance["sku"] == sku]
        ...
        if len(imp):
            ...
            top_features = [
                {"feature": r.feature, "share": round(float(r.importance) / total, 3)}
                for r in imp.itertuples()
            ]
    if not top_features and importance is not None and len(importance):
        imp = (
            importance[importance["sku"] == sku].sort_values("importance", ascending=False).head(5)
        )
        method = "permutation" if len(imp) else None
        total = float(imp["importance"].sum()) or 1.0
        top_features = [
            {"feature": r.feature, "share": round(float(r.importance) / total, 3)}
            for r in imp.itertuples()
    ...
    if abs(fc_vs_recent) > 0.03:
        drivers.append(
            {
                "factor": "forecast vs recent level",
                "direction": "up" if fc_vs_recent > 0 else "down",
    ...
    return {
        ...
        "narrative": (
            ...
            + (
                ...
                f"model carries into the horizon via its lag features."
                if abs(momentum_pct) > 0.03
                else ""
            )
        ).strip(),
...
def explain_recommendation(
    ...
    best_lead = None
    terms = data.supply_terms[
        (data.supply_terms["sku"] == sku) & (data.supply_terms["supplier_id"] == rec.supplier_id)
    ]
    if len(terms):
    ...
    components = [
        ...
        {
            "factor": "pack rounding",
            "units": round(max(rounding, 0), 1),
            "direction": "increase",
            "detail": "rounded up to pack size multiples",
        },
    ]
    for c in components:
    ...
    if rec.supplier_id:
        ...
        if best and runner:
            ...
            if best.get("lead_drift", 0) > 0:
                sup_drivers.append(
                    {
                        "factor": "supplier lead-time drift",
                        "direction": "increase",
...
def to_natural_language(rec: Recommendation, explanation: dict, data) -> str:
    ...
    if rec.action in ("BUY", "EXPEDITE"):
        ...
        parts.append(
            f"Hold {rec.sku} ({rec.product_name}): current inventory already covers "
    ...
    if inc:
        parts.append(
            "Why: the requirement is driven by "
            + ", ".join(f"{c['factor']} (+{c['units']:.0f}u, {c['impact_pct']:.0f}%)" for c in inc)
        )
    if dec:
        parts.append(
            "offset by " + ", ".join(f"{c['factor']} ({c['units']:.0f}u)" for c in dec) + "."
        )
