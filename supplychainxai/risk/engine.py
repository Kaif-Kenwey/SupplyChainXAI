...
def effective_lead(
    sku: str, supply_terms: pd.DataFrame, lt_intel: pd.DataFrame
    ...
    primary = terms[terms["is_primary"].eq(True)]  # nullable-safe
...
def stockout_risk(
    inventory: pd.DataFrame, forecasts: pd.DataFrame, data, z: float = config.SERVICE_LEVEL_Z
) -> list[Alert]:
    alerts = []
    ...
    for sku, st in state.iterrows():
        ...
        sigma_lt = (
            float(
                np.sqrt(
                    (sigma_daily * np.sqrt(horizon)) ** 2 + (mu_lt / max(lead, 1) * lead_std) ** 2
                )
            )
            + 1.0
        )
...
def overstock(
    inventory: pd.DataFrame, forecasts: pd.DataFrame, products: pd.DataFrame
) -> list[Alert]:
    alerts = []
    state = _latest_state(inventory)
...
def demand_spikes(
    sales: pd.DataFrame, baseline_days: int = 60, recent_days: int = 21, min_lift: float = 0.12
) -> list[Alert]:
        ...
        if p_value <= 0.05 and lift >= min_lift:
            alerts.append(
                Alert(
                    f"R3-{sku}",
                    "demand_spike",
                    "CRITICAL" if lift >= 0.30 else "WARNING",
                    sku,
                    None,
        ...
        if drift >= config.LEADTIME_DRIFT_PCT and abs_delta >= 1.5:
            alerts.append(
                Alert(
                    f"R4-{sup}-{sku}",
                    "supplier_lead_drift",
                    "CRITICAL" if drift >= 0.35 else "WARNING",
                    sku,
                    sup,
...
def price_anomalies(
    purchase_orders: pd.DataFrame, z_threshold: float = config.PRICE_ANOMALY_Z
) -> list[Alert]:
    alerts = []
...
def supplier_reliability(purchase_orders: pd.DataFrame, suppliers: pd.DataFrame) -> list[Alert]:
    alerts = []
    ...
    for sup, g in delivered.groupby("supplier_id"):
        ...
        score = 0.6 * on_time + 0.4 * float(
            suppliers.set_index("supplier_id").loc[sup, "target_otd"]
        )
