...
def validate_all(data: RawData) -> ValidationReport:
    ...
    rep.add(
        ...
        "V03 units_sold > 0",
        "CRITICAL",
        int((data.sales["units_sold"] <= 0).sum()),
        "non-positive demand rows",
    )
    rep.add(
        "sales",
        "V04 unit_price > 0",
    ...
    rep.add(
        "sales",
        "V05 calendar coverage >= 98% per sku",
        "WARNING",
        int(coverage.lt(0.98).sum()),
        "skus missing >2% of the daily calendar",
    )
    rep.add(
        "inventory",
        "V06 stock levels >= 0",
    ...
    rep.add(
        ...
        "non-positive PO quantity",
    )
    rep.add(
        "purchase_orders",
        "V08 delivered >= ordered",
        "CRITICAL",
    ...
    rep.add(
        "purchase_orders",
        "V09 every sku has >=1 primary supplier",
        "CRITICAL",
    ...
    rep.add(
        "supply_terms",
        "V10 quoted lead time > 0",
        "CRITICAL",
        int((data.supply_terms["quoted_lead_days"] <= 0).sum()),
    ...
    rep.add(
        ...
        "V11 unique sku-date",
        "CRITICAL",
        int(data.sales.duplicated(subset=["date", "sku"]).sum()),
        "duplicate demand rows",
    )
    rep.add(
        "inventory",
        "V12 unique sku-date",
        "CRITICAL",
        int(data.inventory.duplicated(subset=["date", "sku"]).sum()),
        "duplicate snapshots",
    )
