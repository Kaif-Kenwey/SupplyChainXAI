...
class MovingAverageModel:
    ...
    def _flat(self, level: float, horizon: int) -> pd.DataFrame:
        ...
        return pd.DataFrame({"date": dates, "prediction": max(level, 0.0),
...
class ExponentialSmoothingModel:
    ...
    def predict(self, horizon: int) -> pd.DataFrame:
        ...
        dates = pd.date_range(self.dates.iloc[-1] + pd.Timedelta(days=1), periods=horizon)
        return pd.DataFrame({"date": dates, "prediction": np.clip(f, 0, None),
...
class SARIMAModel:
    ...
    def predict(self, horizon: int) -> pd.DataFrame:
        ...
        dates = pd.date_range(self.dates.iloc[-1] + pd.Timedelta(days=1), periods=horizon)
        return pd.DataFrame({"date": dates, "prediction": mean,
...
class _SklearnModel:
    ...
    def predict(self, horizon: int) -> pd.DataFrame:
        ...
        for h in range(horizon):
            ...
            row = {
                "dow": cal["dow"].iloc[h], "month": cal["month"].iloc[h],
                "weekofyear": cal["weekofyear"].iloc[h],
                "trend_idx": float(self.frame["trend_idx"].iloc[-1]) * (1 + (h + 1) / 1000.0),
                "lag_1": y.iloc[-1], "lag_7": y.iloc[-7] if len(y) >= 7 else y.mean(),
            ...
            hist = pd.concat([hist, pd.DataFrame({
                "date": [cal["date"].iloc[h]], "units_sold": [p],
                "promo_flag": [0], "unit_price": [self.last_price]})], ignore_index=True)
...
def _tree_models():
    ...
    try:  # optional accelerator — same interface if xgboost is installed
        ...
        models.append(("XGBoost", lambda: XGBRegressor(
